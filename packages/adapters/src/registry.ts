...
export class AdapterRegistry {
  ...
  register(adapter: ResourceAdapter): void {
    if (this.adapters.has(adapter.id)) throw new Error(`ADAPTER_ALREADY_REGISTERED:${adapter.id}`);
    this.adapters.set(adapter.id, adapter);
  }

  get(id: string): ResourceAdapter {
    const adapter = this.adapters.get(id);
    if (!adapter) throw new Error(`ADAPTER_NOT_FOUND:${id}`);
    return adapter;
  }

  list(type?: ResourceType): ResourceAdapter[] {
    return [...this.adapters.values()].filter(a => !type || a.resourceType === type);
  }
}
